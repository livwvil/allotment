...
class de extends B {
  ...
  addView(e, t, i, n = this.viewItems.length, r) {
    ...
    const o = this.orientation === ne.Vertical ? new me(e, t, s) : new fe(e, t, s);
    if (this.viewItems.splice(n, 0, o), this.viewItems.length > 1) {
      const _e4 = this.orientation === ne.Vertical ? new ae(this.sashContainer, {
  ...
  layout(e = this.size) {
    ...
    if (this.size = e, this.proportions) for (let _t4 = 0; _t4 < this.viewItems.length; _t4++) {
      ...
      i.size = b(Math.round(this.proportions[_t4] * e), i.minimumSize, i.maximumSize);
    } else {
      const i = G(0, this.viewItems.length),
        n = i.filter(e => this.viewItems[e].priority === ce.Low),
  ...
  resizeViews(e) {
    for (let t = 0; t < e.length; t++) {
      ...
      let n = e[t];
      n = Math.round(n), n = b(n, i.minimumSize, Math.min(i.maximumSize, this.size)), i.size = n;
    }
  ...
  distributeViewSizes() {
    ...
    for (const _t5 of e) _t5.size = b(i, _t5.minimumSize, _t5.maximumSize);
  ...
  resize(e, t, i = this.viewItems.map(e => e.size), n, r, s = Number.NEGATIVE_INFINITY, o = Number.POSITIVE_INFINITY, a, u) {
    ...
    if (r) for (const _e7 of r) R(h, _e7), R(c, _e7);
    if (n) for (const _e8 of n) W(h, _e8), W(c, _e8);
    ...
    if (!S && u) {
      const _e0 = this.viewItems[u.index],
        _i5 = t < u.limitDelta;
      S = _i5 !== _e0.visible, _e0.setVisible(_i5, u.size);
    }
    if (S) return this.resize(e, t, i, n, r, s, o);
    for (let _e1 = 0, _i6 = t = b(t, z, w); _e1 < l.length; _e1++) {
      const _t8 = l[_e1],
        _n2 = b(f[_e1] + _i6, _t8.minimumSize, _t8.maximumSize);
      _i6 -= _n2 - f[_e1], _t8.size = _n2;
    }
    for (let _e10 = 0, _i7 = t; _e10 < m.length; _e10++) {
      const _t9 = m[_e10],
        _n3 = b(d[_e10] - _i7, _t9.minimumSize, _t9.maximumSize);
  ...
  distributeEmptySpace(e) {
    ...
    for (let _e11 = 0; 0 !== i && _e11 < r.length; _e11++) {
      const _t0 = this.viewItems[r[_e11]],
        _n4 = b(_t0.size + i, _t0.minimumSize, _t0.maximumSize);
  ...
  updateSashEnablement() {
    ...
    for (let _e12 = 0; _e12 < this.sashItems.length; _e12++) {
      ...
      const a = !(t[_e12] && s[_e12 + 1]),
        u = !(i[_e12] && r[_e12 + 1]);
      if (a && u) {
        const _i8 = G(_e12, -1, -1),
          _s = G(_e12 + 1, this.viewItems.length),
        ...
        h && r[_e12] && (o > 0 || this.startSnappingEnabled) ? _n5.state = re.Minimum : c && t[_e12] && (o < this.contentSize || this.endSnappingEnabled) ? _n5.state = re.Maximum : _n5.state = re.Disabled;
      } else _n5.state = a && !u ? re.Minimum : !a && u ? re.Maximum : re.Enabled;
    }
  }
  ...
  set preferredSize(e) {
    if ("number" == typeof e) this.layoutStrategy = new pe(e);else if ("string" == typeof e) {
      ...
      if (C(t, "%")) {
        ...
        this.layoutStrategy = new ve(_e15, this.layoutService);
      } else if (C(t, "px")) {
        const _e16 = Number(t.slice(0, -2)) / 100;
        this.layoutStrategy = new pe(_e16);
      } else if ("number" == typeof Number.parseFloat(t)) {
        const _e17 = Number.parseFloat(t);
        this.layoutStrategy = new pe(_e17);
  ...
  constructor(e, t) {
    ...
    if (this.minimumSize = 0, this.maximumSize = Number.POSITIVE_INFINITY, this.element = void 0, this.priority = void 0, this.snap = void 0, this.layoutService = void 0, this.layoutStrategy = void 0, this.layoutService = e, this.element = t.element, this.minimumSize = "number" == typeof t.minimumSize ? t.minimumSize : 30, this.maximumSize = "number" == typeof t.maximumSize ? t.maximumSize : Number.POSITIVE_INFINITY, "number" == typeof t.preferredSize) this.layoutStrategy = new pe(t.preferredSize);else if ("string" == typeof t.preferredSize) {
      ...
      if (C(_e18, "%")) {
        ...
        this.layoutStrategy = new ve(_t1, this.layoutService);
      } else if (C(_e18, "px")) {
        const _t10 = Number(_e18.slice(0, -2));
        this.layoutStrategy = new pe(_t10);
      } else if ("number" == typeof Number.parseFloat(_e18)) {
        const _t11 = Number.parseFloat(_e18);
        this.layoutStrategy = new pe(_t11);
      } else this.layoutStrategy = new ye();
    } else this.layoutStrategy = new ye();
  ...
  return a(O, () => ({
    reset: () => {
      if (z) z();else {
        ...
        for (let e = 0; e < H.current.length; e++) R(e);
      }
    },
    resize: e => {
      var _C$current3;
    ...
    const t = _extends({
      ...
      descriptor: {
        ...
        views: p.map((e, t) => {
          ...
          const i = A.current.get(M.current[t]),
    ...
    C.current = new de(D.current, t, g, x, V), C.current.on("sashDragStart", () => {
      ...
      if (S && C.current) {
        const _e19 = B.map(e => e.key);
        for (let t = 0; t < _e19.length; t++) {
          const i = A.current.get(_e19[t]);
          void 0 !== (i == null ? void 0 : i.visible) && i.visible !== C.current.isViewVisible(t) && S(t, C.current.isViewVisible(t));
        }
      }
    ...
    if ($) {
      ...
      for (let _e20 = r.length - 1; _e20 >= 0; _e20--) {
        var _C$current5;
        r[_e20] && ((_C$current5 = C.current) != null && _C$current5.removeView(_e20), t.splice(_e20, 1), H.current.splice(_e20, 1));
      }
      for (const _n6 of i) {
        var _i9$minSize, _i9$maxSize, _i9$priority, _i9$snap, _C$current6;
        const _i9 = A.current.get(_n6),
          _r2 = new be(k.current, _extends({
            element: document.createElement("div"),
            ...
            priority: (_i9$priority = _i9 == null ? void 0 : _i9.priority) != null ? _i9$priority : ce.Normal
          }, (_i9 == null ? void 0 : _i9.preferredSize) && {
            preferredSize: _i9 == null ? void 0 : _i9.preferredSize
          }, {
            snap: (_i9$snap = _i9 == null ? void 0 : _i9.snap) != null ? _i9$snap : y
          }));
        (_C$current6 = C.current) != null && _C$current6.addView(F.current.get(_n6), _r2, ue.Distribute, e.findIndex(e => e === _n6)), t.splice(e.findIndex(e => e === _n6), 0, _n6), H.current.splice(e.findIndex(e => e === _n6), 0, _r2);
      }
      for (; !w(e, t);) for (const [_i0, _n7] of e.entries()) {
        const _e21 = t.findIndex(e => e === _n7);
        if (_e21 !== _i0) {
          var _C$current7;
          (_C$current7 = C.current) == null || _C$current7.moveView(F.current.get(_n7), _e21, _i0);
          const _r3 = t[_e21];
      ...
      for (const _t12 of i) {
        ...
        const _i1 = e.findIndex(e => e === _t12),
          _n8 = H.current[_i1].preferredSize;
        void 0 !== _n8 && ((_C$current8 = C.current) == null ? void 0 : _C$current8.resizeView(_i1, _n8));
      }
      for (const _t13 of [...i, ...n]) {
        var _C$current9, _C$current0;
        const _i10 = A.current.get(_t13),
          _n9 = e.findIndex(e => e === _t13);
        _i10 && ge(_i10) && void 0 !== _i10.visible && ((_C$current9 = C.current) == null ? void 0 : _C$current9.isViewVisible(_n9)) !== _i10.visible && ((_C$current0 = C.current) == null ? void 0 : _C$current0.setViewVisible(_n9, _i10.visible));
      }
      for (const _t14 of n) {
        const _i11 = A.current.get(_t14),
          _n0 = e.findIndex(e => e === _t14);
        if (_i11 && ge(_i11)) {
          var _C$current1;
          void 0 !== _i11.preferredSize && H.current[_n0].preferredSize !== _i11.preferredSize && (H.current[_n0].preferredSize = _i11.preferredSize);
          let _e22 = !1;
          void 0 !== _i11.minSize && H.current[_n0].minimumSize !== _i11.minSize && (H.current[_n0].minimumSize = _i11.minSize, _e22 = !0), void 0 !== _i11.maxSize && H.current[_n0].maximumSize !== _i11.maxSize && (H.current[_n0].maximumSize = _i11.maxSize, _e22 = !0), _e22 && ((_C$current1 = C.current) == null ? void 0 : _C$current1.layout());
        }
      }
